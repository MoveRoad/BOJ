...
for (let i = 0; i < N; i++) {
  ...
  for (let j = 0; j < operLen; j++) {
    ...
    if (op === "I") {
      ...
      if (num === -1) {
        let temp = min.remove().key;
        numberArr.splice(numberArr.indexOf(temp), 1);
        console.log(Object.values(max.heap).);
      } else {
        numberArr.splice(numberArr.indexOf(min.remove()));
      }
    }
  }
}
